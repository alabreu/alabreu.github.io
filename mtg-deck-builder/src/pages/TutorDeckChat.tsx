  ...
  async function sendMessage(content: string) {
    ...
    try {
      ...
      if (!final.trim()) {
        setMessages((prev) =>
          prev.map((m) =>
            m.id === aId ? { ...m, content: 'O modelo não retornou uma resposta. Tente novamente ou troque de modelo.' } : m
          )
        );
      }
    } catch (err) {
...
const TutorMessageBubble = React.memo(function TutorMessageBubble({
  msg,
  isStreaming,
  onPickCommander,
  onOpenSearch,
